<section class="w-full min-h-[70vh]">
    <div class="max-w-7xl m-auto py-12 px-4 grid grid-cols-2 gap-12 1130px:grid-cols-1 1130px:max-w-2xl">
        <div class="w-full flex flex-col">
            <div class="flex items-center justify-between pb-6 border-b border-gray-200 mb-3">
                <h1 class="text-3xl text-black font-bold">Shopping Cart</h1>
                @if ($total_price != 0)
                    <button class="py-2 px-4 w-fit bg-red-500/20 border border-red-500 text-red-600 rounded-lg mb-3" wire:click='emptyCart'>Empty Cart</button>
                @endif
            </div>
            ...
            <div class="w-full">
                <span class="text-black">Contact (shipping purpose)</span>
                <x-custom-input type="number" name="number" wire:model.debounce.1000ms="number" placeholder="Contact Number" required />
                <x-input-error :messages="$errors->get('number')" class="mt-2" />
            </div>
            <p class="text-gray-600 mb-3">🚚 Estimated Delivery: {{ \Carbon\Carbon::now()->format('d-M-y') }} - {{ \Carbon\Carbon::now()->addDays(5)->format('d-M-y') }}</p>
            @if ($total_price != 0)
                <button wire:click="checkout" class="py-3 px-5 rounded-md text-white text-lg bg-indigo-600 font-semibold">Pay Now</button>
            @endif
        </div>
        <div class="w-full border-l border-gray-200 px-6 1130px:border-none 1130px:px-0">
            @if ($carts != null)
            @foreach ($carts as $cart)
                @if (session('success'))
                    <x-success :message="session('success')" />
                @endif
                <div class="flex items-center justify-between border-b border-gray-200 py-6 mb-3 relative">
                    <div class="flex">
                        ...
                        <div class="p-3">
                            <h3 class="mb-2 text-lg text-black font-semibold">{{ $cart['name'] }}</h3>
                        </div>
                    </div>
                    <span class="text-gray-700 font-bold px-4">${{ $cart['price'] }}</span>
                </div>
            @endforeach
            @endif
            <div class="py-4 px-4 mt-4 rounded-lg bg-light">
                <div class="flex items-center justify-between mb-3">
                    <h3 class="text-gray-300">Shipping Cost</h3>
