    ...
    <div class="max-w-7xl m-auto py-12 px-4 flex gap-12 1130px:grid-cols-1 1130px:max-w-2xl 1130px:flex-col">
        <div class="w-full flex flex-col order-1 1130px:order-2">
            <div class="flex items-center justify-between pb-6 border-b border-white/10 mb-3">
                ...
                @if ($total_price != 0)
                    <button class="py-2 px-4 w-fit bg-red-500/20 border border-red-500 text-white rounded-lg mb-3"
                        wire:click='emptyCart'>Empty Cart</button>
                @endif
            </div>
            <div class="w-full mb-2">
                <span class="text-white">Email Address</span>
                <x-custom-input type="email" name="email" wire:model.debounce.1000ms="email" placeholder="Email Address"
                    required />
            ...
            <div class="grid grid-cols-2 gap-3 mb-2 530px:grid-cols-1">
                <div class="w-full">
                    ...
                    <x-custom-input type="text" name="f_name" wire:model.debounce.1000ms="f_name"
                        placeholder="First Name" required />
                    <x-input-error :messages="$errors->get('f_name')" class="mt-2" />
                </div>
                <div class="w-full">
                    <span class="text-white">Last Name</span>
                    <x-custom-input type="text" name="l_name" wire:model.debounce.1000ms="l_name"
            ...
            <div class="w-full">
                ...
                <x-custom-input type="text" name="address" wire:model.debounce.1000ms="address" placeholder="Address"
                    required />
            ...
            <div class="w-full">
                ...
                <x-custom-input type="number" name="number" wire:model.debounce.1000ms="number"
                    placeholder="Contact Number" required />
                <x-input-error :messages="$errors->get('number')" class="mt-2" />
            </div>
            <p class="text-gray-300 mb-3">🚚 Estimated Delivery: {{ \Carbon\Carbon::now()->format('d-M-y') }} -
                {{ \Carbon\Carbon::now()->addDays(10)->format('d-M-y') }}</p>
            @if ($total_price != 0)
                <button wire:click="checkout"
                    class="py-3 px-5 rounded-md text-black bg-white font-semibold checkout_click">Pay Now</button>
            @endif
        </div>
            ...
            @if ($carts != null)
                @foreach ($carts as $cart)
                    ...
                    <div class="flex items-center justify-between border-b border-white/10 py-6 mb-3 relative">
                        <div class="flex">
                            <img wire:click="removeItem('{{ $cart['slug'] }}')"
                                src="https://api.iconify.design/bi:trash-fill.svg?color=%23ea1f1f"
                                class="absolute p-2 rounded-full bg-red-600/10 -top-0 -left-3" alt="Trash">
                            <img src="{{ $cart['image'] }}" class="w-[60px] object-fit h-[60px] rounded-lg"
                                alt="{{ $cart['name'] }} Image">
                            <div class="p-3">
