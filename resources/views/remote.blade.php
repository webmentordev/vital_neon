        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    @endif
                    <h1 class="font-semibold mb-3">Create Remote Type</h1>
                    <form action="{{ route('remote') }}" method="post" class="flex">
                        @csrf
                        <div class="w-full mr-2">
                            <input type="text" name="type" class="border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm w-full" placeholder="Remote type">
                            @error('type')
                    ...
                    <table class="w-full mt-3 rounded-lg overflow-hidden">
                        ...
                        @foreach ($remotes as $item)
                            <tr class="text-center text-sm">
                                <td class="p-2 text-start">{{ $item->type }}</td>
